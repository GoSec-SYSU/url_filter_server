...
def runAllImageSimilaryFun(para1, para2):
    ...
    if n5 is not list:
        n5 = [n5]
    print("%d %d %d %.2f %.2f " % (n1, n2, n3, np.round(n4[0], 2), n5[0]))
    print("%.2f %.2f %.2f %.2f %.2f " % (1 - float(n1 / 64), 1 -
                                         float(n2 / 64), 1 - float(n3 / 64), np.round(n4[0], 2), n5[0]))
...
def classify_hist_with_split_by_img_path(para1, para2):
    if para1.startswith("http"):
        # 根据链接下载图片，并转换为opencv格式
        img1 = getImageByUrl(para1)
        img1 = cv2.cvtColor(np.asarray(img1), cv2.COLOR_RGB2BGR)
    ...
    n4 = classify_hist_with_split(img1, img2)
    # print('三直方图算法相似度：', n4)
    if type(n4) is not float:
        return np.round(n4[0], 2)
    return round(n4, 2)
...
if __name__ == "__main__":
    ...
    p1 = os.path.join(os.getcwd(), 'files', 'ori.jpg')
    p2 = os.path.join(os.getcwd(), 'files', 'ori.jpg')
    # runAllImageSimilaryFun(p1, p2)
    print(classify_hist_with_split_by_img_path(p1, p2))
