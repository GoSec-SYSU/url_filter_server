...
class Shortest_Url_Handler_Replace:
    ...
    def __init__(self, url):
        ...
        self.random_parameter_val_len = data['random_parameter_val_len']
        self.tmp = []
    ...
    def get_replaced_url(self, doname: str, cur_paras: list, ori_paras: list) -> str:
        if len(ori_paras) == 0:
            return doname
        c_i = 0
        replaced_paras = []
        for o_i in range(len(ori_paras)):
            if c_i < len(cur_paras) and cur_paras[c_i] == ori_paras[o_i]:
                replaced_paras.append(ori_paras[o_i])
            else:
                equals_index = ori_paras[o_i].find('=')
                if equals_index == -1:
                    replaced_paras.append(ori_paras[o_i])
                else:
                    random_str = ''.join(random.sample(string.ascii_letters + string.digits, self.random_parameter_val_len))
                    new_para = ori_paras[o_i][:equals_index] + '=' + random_str + ori_paras[o_i][equals_index + 1:]
                    replaced_paras.append(new_para)
        return doname + '?' + ('&'.join(replaced_paras))
    ...
    def check(self):
        ...
        cur_url = self.get_replaced_url(self.domain, self.tmp, self.arguments)
        ...
        if len(cur_url_screenshot_path) > 200:
            cur_url_screenshot_path = os.path.join(self.screenshot_path,
                                                   ''.join(random.sample(string.ascii_letters + string.digits,
                                                                         self.random_parameter_val_len)) + '.png')
        self.save_url_screenshot(cur_url, cur_url_screenshot_path)
        print('self.ori_url_screenshot_path: ', self.ori_url_screenshot_path)
        print('cur_url_screenshot_path: ', cur_url_screenshot_path)
        ...
        if len(cur_ocr_word) >= self.can_ocr_word_check_word_length and abs(
                len(cur_ocr_word) - len(self.ori_ocr_word)) < 10:
    ...
    def get_filted_url_by_redis(self) -> (bool, str, str):
        ...
        argus_in_redis_list = list(self.r.get(self.redis_key).split('&'))
        # self.redis_key = https://baidu.com?name&age&gender
        # self.arguments = ['name=xinxin', 'age=22', 'gender=male']

        if len(self.arguments) == 0:
            return True, '之前有过了，直接返回', self.url
        c_i = 0
        replaced_paras = []
        for a_i in range(len(self.arguments)):
            equals_index = self.arguments[a_i].find('=')
            if equals_index == -1:
                replaced_paras.append(self.arguments[a_i])
            else:
                argument_name = self.arguments[a_i][:equals_index]
                random_str = ''.join(random.sample(string.ascii_letters + string.digits, self.random_parameter_val_len))
                # 不能修改的值
                if c_i < len(argus_in_redis_list) and argument_name == argus_in_redis_list[c_i]:
                    replaced_paras.append(self.arguments[a_i])
                else:
                    replaced_paras.append(argument_name + '=' + random_str)
        # print('self.domain: ', self.domain)
        return True, '之前有过了，直接返回', self.domain + '?' + ('&'.join(replaced_paras))
...
if __name__ == '__main__':
    ...
    with open('./files/url.txt', 'r', encoding='utf-8') as f:
        url = f.read()
        print(url)
        suh = Shortest_Url_Handler_Replace(url)
